...
export const GroupNode = memo(({ id, data, selected }: { id: string; data: MoodNodeData; selected: boolean }) => {
    ...
    return (
        <div
            className={`h-full w-full flex flex-col transition-all duration-300 ${isCollapsed ? 'overflow-hidden' : ''} pointer-events-none`}
            style={{ zIndex: selected ? 0 : -1 }}
        >
            <div className="pointer-events-auto contents">
                ...
                <Handle
                    ...
                    id="group_in"
                    className={`!w-2 !h-6 !rounded-none !border-none !transition-all hover:!w-3 ${colorClass} !opacity-30 hover:!opacity-100 !pointer-events-auto`}
                    style={{ top: '50%', transform: 'translateY(-50%)' }}
                />

                {/* Group Header */}
                <div className={`flex items-center justify-between px-3 h-8 shrink-0 border-b border-dashed ${borderColor} ${bgColor} backdrop-blur-md pointer-events-auto`}>
                ...
                {/* Content Area */}
                <div className={`flex-1 relative ${isCollapsed ? 'bg-zinc-950/20' : ''} pointer-events-none`}>
                    {!isCollapsed && (
                ...
                {/* Status Footer */}
                <div className={`h-4 border-t border-dashed ${borderColor} flex items-center justify-between px-2 bg-background/40 pointer-events-auto`}>
